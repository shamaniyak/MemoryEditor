...
DynamicLibrary {
	...
	Group {
		...
		qbs.install: true
		qbs.installDir: {
			var p = FileInfo.relativePath(qbs.installRoot, FileInfo.joinPaths(project.globalBinPath, putOverBinSubdir))
			//console.info(p)
			return p
		}
	}
	...
	Group {
		...
		qbs.install: true
		qbs.installDir: {
			var p = FileInfo.relativePath(qbs.installRoot, FileInfo.joinPaths(project.globalBinPath,"qml",product.prefix,product.name))
			//console.info(p)
			return p
		}
	}
